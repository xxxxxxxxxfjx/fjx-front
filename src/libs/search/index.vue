<template>
  <div
    class="group w-full bg-white rounded-xl p-0.5 border-white border relative duration-500 hover:border-red-100 flex hover:bg-red-100/45">
    <div class="w-full">
      <fjx-svg-icon name="search" color="#707070"
        class="w-1.5 h-1.5 absolute top-[50%] translate-y-[-50%] left-2"></fjx-svg-icon>
      <input v-model="inputValue" @focus="inputFocus = true" @blur="inputFocus = false" type="text" class=" block w-full h-[44px] pl-4 outline-0 bg-zinc-100 caret-zinc-400 rounded-xl
         text-zinc-900 tracking-wide font-semibold border border-zinc-100 duration-500 text-sm
        group-hover:bg-white group-hover:border-zinc-200 focus:border-red-300" placeholder="搜索">
    ...
    <transition name="slide">
      <div v-show="inputFocus" v-if="slots.dropdown"
        class=" max-h-[368px] w-full text-base overflow-auto bg-white absolute z-20 left-0 top-[56px] p-2 rounded border border-zinx-200 duration-200 hover:shadow-3xl">
        <slot name="dropdown" />
      </div>
    </transition>
  </div>
