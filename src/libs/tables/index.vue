  ...
  <div class="gw-table-content">
    ...
    <table class="gw-fixed-table" v-if="tableHeaderFixed">
      ...
      fontSize: headerFontSize || '18px'
        ...
        <th :class="{ ellipsis: headerEllipsis }" :width="h.width" v-for="(h, i) in columns" :key="i">
          {{ h.label }}
        </th>
      </thead>
    </table>
    <div class="un-fixed-table-box">
      <table class="gw-table">
        <!-- 正常的表头，无固定 -->
        <thead v-if="!tableHeaderFixed" :style="{
      backgroundColor: headerBackground
        ? 'hsla(200, 79%, 49%, 0.2)'
        : 'transparent',
      fontSize: headerFontSize || '18px'
        ...
        <tbody>
          <tr :class="{ dash: trDash }" v-for="(tr, ri) in dataSource" :key="ri">
            <!-- 是否需要序号 -->
      ...
      padding: tdPadding,
      width: td.width
    }" v-for="(td, key) in columns" :key="key">
